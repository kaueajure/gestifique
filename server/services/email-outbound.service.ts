  ...
  async sendTicketEmail(params: TicketOutboundParams): Promise<TicketEmailSendResult> {
    ...
    if (
      ...
      if (gmailResult.success) {
        console.log(
          `[EmailOutbound] Ticket email (${params.type}) sent via Gmail to ${params.to} (ticket #${params.ticketId})`
        );
        return {
          success: true,
          messageId: msgId,
      ...
      console.warn(
        `[EmailOutbound] Gmail failed for ticket #${params.ticketId}, using SMTP fallback: ${gmailResult.error}`
      );
    }
