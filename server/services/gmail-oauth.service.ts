...
function createOAuth2Client() {
  ...
  return new google.auth.OAuth2(
    env.GOOGLE_OAUTH.CLIENT_ID,
    env.GOOGLE_OAUTH.CLIENT_SECRET,
    env.GOOGLE_OAUTH.REDIRECT_URI
  );
}

function signOAuthState(payload: GmailOAuthStatePayload): string {
...
function verifyOAuthState(state: string): GmailOAuthStatePayload {
  ...
  const payload = JSON.parse(Buffer.from(body, 'base64url').toString('utf8')) as GmailOAuthStatePayload;
...
export class GmailOAuthService {
  ...
  static getAuthUrl(channelId: number, empresaId: number, userId: number): string {
  ...
  static async handleCallback(code: string, state: string): Promise<{
    ...
    const channel = await emailChannelsService.getByIdAndCompany(
      statePayload.channelId,
      statePayload.empresaId
    );
    ...
    if (oauthEmail !== publicEmail) {
      ...
      throw new Error(
        `A conta conectada (${oauthEmail}) não corresponde ao e-mail do canal (${publicEmail}). ` +
    ...
    await emailChannelsService.logChannelEvent(
      statePayload.empresaId,
      'GMAIL_OAUTH_CONNECTED',
      `Gmail conectado para envio no canal #${statePayload.channelId} (${oauthEmail})`,
      statePayload.userId
    );
  ...
  static async getValidAccessToken(channelId: number, empresaId: number): Promise<string> {
  ...
  static async refreshAccessToken(channel: any): Promise<string> {
    const client = createOAuth2Client();
    ...
    try {
      ...
      if (isInvalidGrant) {
        ...
        await emailChannelsService.logChannelEvent(
          channel.empresa_id,
          'GMAIL_OAUTH_EXPIRED',
          `Token OAuth expirado/revogado no canal #${channel.id}: ${message}`
        );
      } else {
  ...
  static async sendEmail(
    ...
    options?: { updateChannelError?: boolean }
  ...
  static async disconnect(channelId: number, empresaId: number, userId?: number): Promise<void> {
    const channel = await emailChannelsService.getByIdAndCompany(channelId, empresaId);
    if (!channel) {
      throw new Error('Canal de e-mail não encontrado');
    }
    ...
    await emailChannelsService.logChannelEvent(
      ...
      `Gmail desconectado do canal #${channelId}`,
      userId
    );
  }
  ...
  static async sendTestEmail(
    ...
    await emailChannelsService.updateSendTestResult(
      channelId,
      empresaId,
