...
export type TicketStatusSpecial = typeof TICKET_STATUS_SPECIALS[number];
...
export interface TicketStatusConfig {
  ...
  valor: string;
  ativo: number;
  kanban_visivel: number;
  cor: string | null;
  especial: TicketStatusSpecial;
  ordem: number;
}
...
export function isValidTicketStatusValue(value: unknown): value is string {
  return typeof value === 'string' && /^[a-z0-9_]{2,80}$/.test(value);
}

export function normalizeTicketStatusSpecial(value: unknown): TicketStatusSpecial {
  return TICKET_STATUS_SPECIALS.includes(value as TicketStatusSpecial)
    ? value as TicketStatusSpecial
    : 'normal';
}

export function isFinalTicketStatusSpecial(special: unknown): boolean {
  return special === 'finalizado' || special === 'encerrado';
}

export function isCustomerWaitingTicketStatusSpecial(special: unknown): boolean {
...
export function buildStatusInCondition(column: string, statuses: string[], negate = false): { sql: string; params: string[] } {
  const safeStatuses = statuses.filter(isValidTicketStatusValue);
  if (safeStatuses.length === 0) return { sql: '', params: [] };
