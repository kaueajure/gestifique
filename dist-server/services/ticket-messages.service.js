...
class TicketMessagesService {
    ...
    async addMessage(data, currentUser) {
        ...
        const [ticketRows] = await pool.query(`SELECT t.*, 
       ...
       WHERE t.id = ?`, [ticket_id]);
        ...
        if (currentUser) {
            const [recentSameUserMessage] = await pool.query(`SELECT id
         FROM ticket_mensagens
         WHERE ticket_id = ?
           AND usuario_id <=> ?
           AND interno = ?
         ...
         ORDER BY id DESC
         LIMIT 1`, [ticket_id, usuario_id || null, finalInterno ? 1 : 0, tipo, mensagem]);
