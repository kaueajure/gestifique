...
class ReportsService {
    async getDashboardStats(user) {
       ...
       FROM tickets ${where}`, params);
        ...
        const [byStatus] = await pool.query(`SELECT status, COUNT(*) as qtd FROM tickets ${where} GROUP BY status`, params);
        stats.byStatus = byStatus;
        const [byPriority] = await pool.query(`SELECT prioridade, COUNT(*) as qtd FROM tickets ${where} GROUP BY prioridade`, params);
        stats.byPriority = byPriority;
        const [recentTickets] = await pool.query(`SELECT t.*, u.nome as cliente_nome 
       FROM tickets t
       LEFT JOIN usuarios u ON t.usuario_id = u.id
       ${where} 
       ORDER BY t.created_at DESC LIMIT 5`, params);
        stats.recentTickets = recentTickets;
        const [atividades] = await pool.query(`SELECT l.*, u.nome as usuario_nome 
       FROM logs_sistema l
       JOIN usuarios u ON l.usuario_id = u.id
       ${!isDev ? (isAdmin ? 'WHERE u.empresa_id = ?' : 'WHERE l.usuario_id = ?') : ''}
       ORDER BY l.created_at DESC LIMIT 10`, !isDev ? (isAdmin ? [empresaId] : [userId]) : []);
        stats.recentActivities = atividades;
        return stats;
    }
    async getPerformance(empresaId) {
        const where = empresaId ? 'WHERE empresa_id = ?' : '';
        const [topUsers] = await pool.query(`SELECT u.nome, COUNT(t.id) as resolvidos
       FROM usuarios u
       JOIN tickets t ON u.id = t.responsavel_id
       WHERE t.status IN ('resolvido', 'fechado')
       ${empresaId ? 'AND u.empresa_id = ?' : ''}
       GROUP BY u.id ORDER BY resolvidos DESC LIMIT 5`, empresaId ? [empresaId] : []);
        return { topUsers };
    }
}
