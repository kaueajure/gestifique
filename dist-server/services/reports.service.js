...
class ReportsService {
    ...
    `, params);
        ...
        const [csatRows] = await pool.query(`
      SELECT 
    ...
    `, params);
        ...
        let resParamsList = [`${startDate} 00:00:00`, `${endDate} 23:59:59`];
        if (filters.empresa_id) {
            resClausesList.push('empresa_id = ?');
            resParamsList.push(filters.empresa_id);
        }
        if (filters.responsavel_id) {
            resClausesList.push('responsavel_id = ?');
            resParamsList.push(filters.responsavel_id);
        }
        if (filters.status) {
            resClausesList.push('status = ?');
            resParamsList.push(filters.status);
        }
        if (filters.prioridade) {
            resClausesList.push('prioridade = ?');
            resParamsList.push(filters.prioridade);
        }
        if (filters.categoria) {
            resClausesList.push('categoria = ?');
            resParamsList.push(filters.categoria);
        }
        if (filters.servico) {
            resClausesList.push('servico = ?');
            resParamsList.push(filters.servico);
        }
        if (filters.origem) {
            resClausesList.push('origem = ?');
            resParamsList.push(filters.origem);
        }
        ;
        const [resDayRows] = await pool.query(`
      SELECT 
    ...
    `, resParamsList);
        ...
        return {
            ...
            by_responsible: responsibleRows.map((r) => ({ name: r.name || 'Sem Responsável', value: Number(r.total), avg_res: Math.round(r.avg_res * 10) / 10 })),
            by_day,
            rankings: {
                top_agents: responsibleRows.slice(0, 5).map((r) => ({ name: r.name || 'N/A', value: Number(r.total) })),
