...
export class EmailListenerService {
    ...
    static async findPersistedEmailTicket(messageId, empresaId) {
        if (!messageId)
            return null;
        const [ticketRows] = await pool.query('SELECT id FROM tickets WHERE message_id = ? AND empresa_id = ? ORDER BY id ASC LIMIT 1', [messageId, empresaId]);
                ...
                try {
                    ...
                    const identifyTicket = async (companyId) => {
                        ...
                        if (headerTicketIdStr && typeof headerTicketIdStr === 'string' && !isNaN(parseInt(headerTicketIdStr))) {
                            ...
                            const [rows] = await pool.query('SELECT id, empresa_id FROM tickets WHERE id = ? AND empresa_id = ? LIMIT 1', [id, companyId]);
                            if (rows.length > 0) {
                        ...
                        for (const candidate of candidates) {
                            ...
                            if (extractedTicketId) {
                                const [rows] = await pool.query('SELECT id, empresa_id FROM tickets WHERE id = ? AND empresa_id = ? LIMIT 1', [extractedTicketId, companyId]);
                                if (rows.length > 0) {
