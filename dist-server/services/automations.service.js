...
function getTicketField(ticket, campo) {
    switch (campo) {
...
async function evaluateCondition(ticket, cond) {
    ...
    if (cond.campo === 'tag') {
        ...
        if (!tags) {
            const [rows] = await pool.query('SELECT tag FROM ticket_tags WHERE ticket_id = ?', [ticket.id]);
...
export async function runAutomations(evento, ticket, contexto) {
    if (contexto?.isInternalAutomation)
        return;
    ...
    if (!ticket._automationsProcessed)
    ...
    try {
        ...
        for (const regra of regras) {
            ...
            if (ticket._automationsProcessed.has(ruleKey))
                continue;
