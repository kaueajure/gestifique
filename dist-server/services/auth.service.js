...
class AuthService {
    ...
    async forgotPassword(email) {
        ...
        await pool.query('UPDATE usuarios SET reset_token = ?, reset_token_expires = ? WHERE id = ?', [token, expires, user.id]);
        await sendPasswordRecoveryEmail(email, token);
        return { message: 'Token de recuperação enviado para o seu e-mail.' };
    }
    async resetPassword(email, token, newPassword) {
        const [rows] = await pool.query('SELECT * FROM usuarios WHERE email = ? AND reset_token = ? AND reset_token_expires > NOW()', [email, token]);
        if (rows.length === 0) {
