...
class AttachmentsService {
    ...
    async getById(id) {
        const [rows] = await pool.query('SELECT a.*, t.empresa_id as ticket_empresa_id FROM ticket_anexos a JOIN tickets t ON a.ticket_id = t.id WHERE a.id = ?', [id]);
        const data = rows[0];
        if (!data)
            return null;
        return {
    ...
    async create(data) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [ticket_id, mensagem_id || null, usuario_id, empresa_id, nome_original, nome_arquivo, caminho, mime_type, tamanho_bytes, interno ? 1 : 0]);
        ...
        // Notificações
        try {
            const [ticketRows] = await pool.query('SELECT * FROM tickets WHERE id = ?', [ticket_id]);
