...
async function ensureStatusTransitionPermission(res, currentUser, empresaId, oldStatus, newStatus) {
...
async function hasAnyTicketPermission(currentUser, permissionKeys) {
    for (const permissionKey of permissionKeys) {
        if (await permissionsService.hasPermission(currentUser, permissionKey))
            return true;
    }
    return false;
}
async function getBulkActionPermissionError(currentUser, action, value) {
    switch (action) {
        case 'status': {
            const hasStatusPerm = await permissionsService.hasPermission(currentUser, 'tickets.editar_status');
            if (!hasStatusPerm)
                return 'Acesso proibido: Sem permissao para alterar status em massa (tickets.editar_status).';
...
router.get('/', requirePermission('tickets.visualizar'), async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
...
router.get('/kanban', requirePermission('tickets.visualizar'), async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
...
router.patch('/bulk', async (req, res) => {
    try {
        ...
        await logSystemAction(req, currentUser.id, currentUser.empresa_id, 'TICKET_BULK_ACTION', `Ação em massa (${action}) processada para ${result.updated} chamados (${result.skipped} ignorados)`);
...
router.post('/views', async (req, res) => {
    try {
        ...
        if (!nome)
            return sendError(res, 'Nome da view é obrigatório', 400);
        if (nome.length > 100)
            return sendError(res, 'Nome da view muito longo (máx 100 caracteres)', 400);
        if (!filtros_json || typeof filtros_json !== 'object')
            return sendError(res, 'Filtros inválidos', 400);
        let targetEmpresaId = currentUser.empresa_id;
        if (currentUser.desenvolvedor) {
            const bodyEmpresaId = toPositiveInt(req.body.empresa_id);
            if (!bodyEmpresaId)
                return sendError(res, 'Selecione uma empresa antes de salvar uma view.', 400);
        ...
        if (!targetEmpresaId)
            return sendError(res, 'Empresa inválida', 400);
...
router.patch('/:id/resolve', async (req, res) => {
    try {
        ...
        const id = parseInt(req.params.id);
        const ticket = await ticketsService.getByIdForUser(id, currentUser);
        if (!ticket)
            return sendError(res, 'Ticket não encontrado', 404);
        if (ticket.error === 'forbidden')
            return sendError(res, 'Permissão negada', 403);
...
router.post('/', requirePermission('tickets.criar'), async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
        ...
        if (!titulo || titulo.trim().length < 3)
            return sendError(res, 'Título obrigatório (min 3 caracteres)', 400);
        if (!descricao || descricao.trim().length < 5)
            return sendError(res, 'Descrição obrigatória (min 5 caracteres)', 400);
        ...
        const [empresaRows] = await pool.query('SELECT ativo FROM empresas WHERE id = ?', [targetEmpresaId]);
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const hasFieldChanged = (field) => Object.prototype.hasOwnProperty.call(req.body, field) &&
            String(req.body[field] ?? '') !== String(ticket[field] ?? '');
        const ensureFieldPermission = async (permission, message) => {
        ...
        if (hasFieldChanged('titulo')) {
            const denied = await ensureFieldPermission('tickets.editar_titulo', 'Acesso proibido: Sem permissao para editar titulo (tickets.editar_titulo).');
            if (denied)
                return denied;
        }
        if (hasFieldChanged('descricao')) {
            const denied = await ensureFieldPermission('tickets.editar_descricao', 'Acesso proibido: Sem permissao para editar descricao (tickets.editar_descricao).');
            if (denied)
                return denied;
        }
        if (hasFieldChanged('status')) {
            const denied = await ensureFieldPermission('tickets.editar_status', 'Acesso proibido: Sem permissao para alterar status (tickets.editar_status).');
            if (denied)
                return denied;
        ...
        if (hasFieldChanged('origem')) {
            const denied = await ensureFieldPermission('tickets.editar_origem', 'Acesso proibido: Sem permissao para editar origem (tickets.editar_origem).');
            if (denied)
                return denied;
        }
        if (hasFieldChanged('prazo_sla')) {
            const denied = await ensureFieldPermission('tickets.alterar_sla', 'Acesso proibido: Sem permissao para alterar SLA (tickets.alterar_sla).');
            if (denied)
                return denied;
        }
        ...
        if (req.body.responsavel_id !== undefined && req.body.responsavel_id !== null && req.body.responsavel_id !== '') {
            ...
            const newRespId = toPositiveInt(req.body.responsavel_id);
            if (newRespId) {
                const [respUser] = await pool.query('SELECT empresa_id FROM usuarios WHERE id = ?', [newRespId]);
                if (!respUser[0])
                    return sendError(res, 'Usuário responsável não encontrado', 404);
        ...
        let descriptions = [];
        if (req.body.prioridade && req.body.prioridade !== oldPrio)
...
router.get('/:id/messages', async (req, res) => {
    try {
        ...
        if (!canView)
            return sendError(res, 'Acesso proibido: Sem permissão para visualizar mensagens (ticket_mensagens.visualizar).', 403);
...
router.get('/:id/timeline', async (req, res) => {
    try {
        ...
        const canView = await permissionsService.hasPermission(currentUser, 'ticket_mensagens.visualizar');
        if (!canView)
            return sendError(res, 'Acesso proibido: Sem permissão para visualizar timeline (ticket_mensagens.visualizar).', 403);
        const hasVerInternos = await permissionsService.hasPermission(currentUser, 'ticket_mensagens.ver_internos');
...
router.post('/:id/messages', async (req, res) => {
    try {
        ...
        if (ticketResult.error === 'forbidden')
            return sendError(res, 'PermissÃ£o negada', 403);
        if (isInternalCom) {
...
router.post('/:id/attachments', ticketUpload.array('files', 5), async (req, res) => {
    ...
    try {
        ...
        if (mensagem_id) {
            ...
            const [msgRows] = await pool.query('SELECT ticket_id FROM ticket_mensagens WHERE id = ?', [msgIdNum]);
...
router.post('/:id/tags', requirePermission('tickets.gerenciar_tags'), async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
        ...
        const { tag } = req.body;
        if (!tag)
            return sendError(res, 'Tag é obrigatória', 400);
        const result = await ticketsService.getByIdForUser(id, currentUser);
        if (!result)
            return sendError(res, 'Ticket não encontrado', 404);
        if (result.error === 'forbidden')
...
router.put('/:id/tags', requirePermission('tickets.gerenciar_tags'), async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
        ...
        if (!Array.isArray(tags))
            return sendError(res, 'Tags devem ser um array', 400);
        const result = await ticketsService.getByIdForUser(id, currentUser);
        if (!result)
            return sendError(res, 'Ticket não encontrado', 404);
        if (result.error === 'forbidden')
...
router.delete('/:id/tags/:tag', requirePermission('tickets.gerenciar_tags'), async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
...
router.put('/:id/custom-fields', requirePermission('tickets.editar'), async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
        ...
        if (!Array.isArray(fields))
            return sendError(res, 'Campos devem ser um array', 400);
        const result = await ticketsService.getByIdForUser(id, currentUser);
        if (!result)
            return sendError(res, 'Ticket não encontrado', 404);
        if (result.error === 'forbidden')
