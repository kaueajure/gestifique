...
async function getBulkActionPermissionError(currentUser, action, value) {
    switch (action) {
        case 'status': {
            ...
            if (!hasStatusPerm)
                return 'Voce nao tem permissao para alterar status em massa.';
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        if (hasFieldChanged('status')) {
            const denied = await ensureFieldPermission('tickets.editar_status', 'Voce nao tem permissao para alterar status.');
            if (denied)
                return denied;
