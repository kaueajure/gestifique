...
router.get('/summary', requirePermission('dashboard.visualizar'), async (req, res) => {
    try {
      ...
      `, scopedParams);
        ...
        const [priorityResult] = await pool.query(`SELECT COALESCE(prioridade, 'sem_prioridade') as prioridade, COUNT(*) as qtd FROM tickets WHERE 1=1 ${ticketCompanyFilter} GROUP BY prioridade`, scopedParams);
