...
router.post('/request-code', authRateLimit, async (req, res) => {
    ...
    `, [orgEmail, orgEmail, orgEmail, orgEmail]);
        ...
        await pool.query('UPDATE portal_access_codes SET used_at = NOW() WHERE empresa_id = ? AND customer_email = ? AND used_at IS NULL', [company.id, custEmail]);
        // 4. Save new code
        await pool.query(`
...
router.post('/verify-code', authRateLimit, async (req, res) => {
    ...
    `, [company.id, custEmail]);
        ...
        const [userRows] = await pool.query('SELECT id, nome, email, empresa_id FROM usuarios WHERE LOWER(email) = ? AND empresa_id = ? AND ativo = 1 LIMIT 1', [custEmail, company.id]);
