...
router.get('/tickets/:id/summary', authMiddleware, requirePermission('ia.usar_resumo'), async (req, res) => {
...
router.post('/tickets/:id/suggest-reply', authMiddleware, requirePermission('ia.sugerir_resposta'), async (req, res) => {
    const ticketId = Number(req.params.id);
...
router.post('/chat', authMiddleware, requirePermission('ia.chat'), async (req, res) => {
