...
router.get('/companies/:companyId/email-channels/:id/oauth/google/start', async (req, res) => {
    try {
        const companyId = parseInt(req.params.companyId, 10);
...
router.post('/companies/:companyId/email-channels/:id/oauth/google/disconnect', async (req, res) => {
    try {
        const companyId = parseInt(req.params.companyId, 10);
        const channelId = parseInt(req.params.id, 10);
        if (!canManage(req, companyId)) {
...
router.post('/companies/:companyId/email-channels/:id/oauth/google/test-send', async (req, res) => {
    try {
        const companyId = parseInt(req.params.companyId, 10);
        const channelId = parseInt(req.params.id, 10);
        if (!canManage(req, companyId)) {
