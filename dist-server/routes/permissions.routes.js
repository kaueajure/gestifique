...
router.get('/catalog', requirePermission('usuarios.ver_permissoes'), async (req, res) => {
    try {
...
router.post('/users/:id/bulk', requirePermission('usuarios.gerenciar_permissoes'), async (req, res) => {
    try {
        ...
        const [validRows] = await pool.query('SELECT permission_key FROM permissions_catalog WHERE permission_key IN (?) AND ativo = 1', [permission_keys]);
        const validKeys = validRows.map((r) => r.permission_key);
...
router.post('/users/:id/bulk-reset', requirePermission('usuarios.gerenciar_permissoes'), async (req, res) => {
    try {
        ...
        // Validate permission_keys exist in permissions_catalog
        const [validRowsReset] = await pool.query('SELECT permission_key FROM permissions_catalog WHERE permission_key IN (?) AND ativo = 1', [permission_keys]);
        const validKeysReset = validRowsReset.map((r) => r.permission_key);
